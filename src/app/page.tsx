...
import { CheckCircle, ShieldCheck, XCircle, Zap } from "lucide-react";

const PainPoint = ({ children }: { children: React.ReactNode }) => (
  <div className="flex items-start gap-3">
    <XCircle className="w-5 h-5 text-rose-400 mt-1 shrink-0" />
...
const FloatingLabel = ({ text, className }: { text: string; className: string }) => (
    <div className={`absolute p-4 rounded-full text-xs font-bold bg-slate-900/50 text-white backdrop-blur-sm border border-slate-700 whitespace-nowrap shadow-lg ${className}`}>
