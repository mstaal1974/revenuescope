...
const AuditWidget: React.FC = () => {
  ...
  const handleAudit = async (e?: React.FormEvent, override?: {auditType: 'qual' | 'rto', code: string}) => {
    ...
    try {
      ...
      // Auto-populate from previous lead if available
      const savedName = localStorage.getItem('leadName') || '';
      const savedEmail = localStorage.getItem('leadEmail') || '';
      const savedPhone = localStorage.getItem('leadPhone') || '';
      setLeadForm({ name: savedName, email: savedEmail, phone: savedPhone });
  ...
  const handleLeadSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Persist lead ID and details for session management
      localStorage.setItem('leadId', leadId);
      localStorage.setItem('leadName', leadForm.name);
