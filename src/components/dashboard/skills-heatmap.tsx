    ...
    <div className="w-full bg-slate-900 rounded-[2.5rem] p-8 md:p-12 border border-slate-800 overflow-hidden relative shadow-2xl">
      ...
      <div className="flex flex-col gap-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {tiers.map((tier) => {
            const isHighDemand = tier.demand_level.toLowerCase().includes('high');
            ...
            return (
              <div key={tier.tier_level} className={cn(`
                ...
                p-8 hover:bg-slate-800/50 transition-all duration-500 group flex flex-col min-h-[400px]
              `)}>
                ...
                <div className="flex justify-between items-start mb-6">
                  <div className={cn(`
                    ...
                    <Activity size={12} />
                    {tier.demand_level} Demand
                  </div>
                  <span className="text-slate-500 font-mono text-[10px] font-bold tracking-widest">{tier.match_percentage}% MATCH</span>
                </div>

                <h4 className="text-2xl font-black text-white mb-3 group-hover:text-blue-300 transition-colors tracking-tight">
                  {tier.title}
                </h4>
                <p className="text-sm text-slate-400 mb-8 leading-relaxed font-medium flex-grow">
                  {tier.marketing_hook}
                </p>
                ...
                <div className="space-y-4 relative mb-8">
                  ...
                  {tier.included_units.map((unit, idx) => (
                    <div key={idx} className="relative z-10 flex items-center gap-4">
                      <div className={cn(`
                        w-6 h-6 rounded-full border-2 flex items-center justify-center bg-slate-950 shrink-0 transition-colors`,
                        idx === 0 ? 'border-emerald-500 text-emerald-500' : 'border-slate-700 text-slate-500'
                      )}>
                        <span className="text-[10px] font-black">{idx + 1}</span>
                      </div>

                      <div className="flex-1 p-3 rounded-xl bg-slate-900/80 border border-slate-800 group-hover:border-slate-700 transition-all">
                        <div className="flex justify-between items-center gap-3">
                          <span className="text-slate-200 text-xs font-bold truncate">{unit.name}</span>
                          <span className="text-[8px] font-black text-slate-500 uppercase border border-slate-800 px-1.5 py-0.5 rounded shrink-0 tracking-tighter bg-black/20">
                            {unit.type}
                          </span>
                        </div>
                      </div>
